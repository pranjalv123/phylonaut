...
vector<Clade> ConsensusTreeAnalysis::children(Clade& c, vector<Clade>& clades) {
  ...
  for (Clade& other : clades) {
    ...
    if (remaining.size() == 0) {
      // /* DEBUG */ LOG(INFO) << "Clade " << c.str() << " has " << clist.size() << " children" << endl;
      return clist;
    }
  }

  
  LOG(ERROR) << "Clade missing some subclades" << endl;
  LOG(ERROR) << remaining.size() << "/" << c.size() << endl;
  assert(false);
}
...
string ConsensusTreeAnalysis::analyze(TaxonSet& ts, vector<Clade>& clades, TripartitionScorer& tps) {
  ...
  // /* DEBUG */ LOG(INFO) << in_optimal.size() << endl;
  ...
  for (Clade& c : clades) {
    ...
    if (c.get_taxa() == ts.taxa_bs || (appearances[c.get_taxa()] > (level * total_trees) || (level==1.0 && appearances[c.get_taxa()] == (total_trees)) || (level<=0))) {
      // /* DEBUG */ LOG(INFO) << (double)appearances[c.get_taxa()] << "\t" << (double)total_trees << endl;
  ...
  // /* DEBUG */ LOG(INFO) << "Found " << good_clades.size() << " clades that appear in at least " << level << " trees" << endl;
  ...
  // /* DEBUG */ LOG(INFO) << "Found " << compatible.size() << " compatible clades" << endl;
  ...
  for (Clade& c : compatible ) {
    ...
    // /* DEBUG */ LOG(INFO) << c.size() << "\t" << kids[c.get_taxa()].size() << endl;
  }
  ...
  for (Clade& c : compatible ) {
    ...
    // /* DEBUG */ LOG(INFO) << c.str() << "\t" << support << "\t" << kids[c.get_taxa()].size() << endl;
