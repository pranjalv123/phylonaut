...
double TripartitionScorer::get_score(Clade& clade, int clade_index) {
  ...
  // /* DEBUG */ LOG(INFO) << "SCORING TRIPARTITION " << clade_index << endl;
  ...
  // /* DEBUG */ LOG(INFO) << clade.str() << "\t" << get_subclades(clade).size() << endl;
  return value;
}
...
vector<pair<clade_bitset, clade_bitset>>& TripartitionScorer::get_subclades(const clade_bitset& clade) {
  if(clade_indices.count(clade) == 0){
    Clade c(ts(), clade);
    LOG(ERROR) << c.str() << " doesn't have subclades!" << endl;
    assert(false);
  }
...
void TripartitionScorer::init(Config& conf) {
  // /* DEBUG */ LOG(INFO) << clades.size() << endl;
  ...
  if (conf.matrix) {
    // /* DEBUG */ LOG(INFO) << "Making score matrix" << endl;
